...
| **LEDGER** | `DecisionLedgerViewer.tsx` | **YES** | **NO** | **NO** | Consumes `GET /api/investigations/{case_id}/decisions` and `POST /api/ledger/verify?case_id={caseId}`. Live SHA-256 Merkle-style block hashes are validated dynamically against `decision_ledger.db`. |
